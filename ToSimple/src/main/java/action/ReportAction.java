...
public class ReportAction {
    ...
    public String save(HttpServletResponse response, @RequestParam(value = "questionnaireId", defaultValue = "") String questionaireId, String content) throws IOException {
        System.out.print("questionID:::");
        System.out.print(questionaireId);
        System.out.print(content);
        JSONObject jsonObject = new JSONObject();
        if (questionaireId.equals("")) {
            jsonObject.put("valid", 0);
            response.getWriter().print(jsonObject);
            return null;
        } else {
            Report report1 = new Report(questionaireId, new Date(), 0, content);
